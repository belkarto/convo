    ...
    try:
        ...
        for i in range(1, 200):
            user = get_or_create_user(
                username=f"user{i}", email=f"user{i}@mail.com", password="1234"
            )
            ...
            try:
                Friendship.objects.get_or_create(
                    sender=sender, receiver=receiver, defaults={"status": status}
                )
            except IntegrityError:
                print(
                    f"Skipped duplicate friendship for sender={sender} and receiver={receiver}"
                )
